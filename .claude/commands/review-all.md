Review the entire project codebase. Use `find . -name "*.py" -not -path "./.venv/*" -not -path "./__pycache__/*"` and `find ./frontend/src -name "*.vue" -o -name "*.js"` to get all project source files.

For each category below, run a parallel subagent that reads the relevant files and reports findings as a bullet list. Only report actual issues, not generic advice.
...
- Missing authentication/authorization checks on endpoints or handlers
...
## 4. Testing
- Logic paths without corresponding tests
...
- Environment variables or config not documented in `rules/project.md`
