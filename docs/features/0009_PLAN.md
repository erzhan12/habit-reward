...
> **Note (2026-02):** The `STREAK_MULTIPLIER_RATE` setting referenced in this plan has been replaced by `STREAK_REDUCTION_RATE` and `MIN_NO_REWARD_PROBABILITY`. See migration 0026 and the current `src/habit_reward_project/settings.py`.

## Overview

Migrate the habit reward bot from Airtable-based architecture to a Django framework (SQLite by default, including current production; PostgreSQL optional later). Replace Telegram polling with webhooks for improved scalability and resource efficiency.
...
```
src/
├── habit_reward_project/          # Django project root
│   ├── __init__.py
│   ├── settings.py                # Django settings (replaces src/config.py)
│   ├── urls.py                    # URL routing
│   ├── wsgi.py                    # WSGI entry point
│   └── asgi.py                    # ASGI entry point (for webhooks)
├── core/                          # Django app for core models
│   ├── models.py                  # Django models (replaces src/models/)
...
```toml
django = ">=5.0"
# (Optional) Only needed if/when migrating to PostgreSQL
...
- `id: str | None` → Auto-generated `id` primary key (Django default)
- `telegram_id: str` → `CharField(unique=True, db_index=True)` for fast lookups
- `active: bool = False` → `BooleanField(default=False)` - security by default
...
**Mapping**:
- `weight: int = 10` → `IntegerField` with validators (1-100 range)
- `active: bool = True` → Soft delete support
...
**Mapping**:
- `type: RewardType` → Django `TextChoices` enum
- `piece_value: float | None` → `DecimalField` (better for monetary values)
- `pieces_required: int = 1` → All rewards are cumulative (unified system)
...
**Mapping**:
- `user_id: str` → `ForeignKey(User, on_delete=CASCADE)`
- `reward_id: str` → `ForeignKey(Reward, on_delete=CASCADE)`
- `status` → `@property` computed field (replaces Airtable formula)
- `pieces_required` → `@property` lookup via FK (replaces Airtable linked field)
...
**Mapping**:
- All `_id` fields → Django `ForeignKey`
- `reward_id: str | None` → `ForeignKey(Reward, on_delete=SET_NULL, null=True)`
...
**Production**:
- [ ] (Optional) Switch `DATABASE_URL` to PostgreSQL
- [ ] Run migrations on production DB
- [ ] Configure reverse proxy (Caddy in current deployment)
