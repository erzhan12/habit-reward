...
Buttons:
- "✅ Today" → `habit_{habit_id}_today`
- "📅 Yesterday" → `habit_{habit_id}_yesterday`
- "📆 Select Date" → `backdate_habit_{habit_id}`
- "« Back" → `menu_back`
...
Nov 27   | Nov 28   | Nov 29   | Nov 30 (today)
        « Back
```
- Dates with checkmarks (✓) are already completed (disabled/show different callback)
...
Buttons:
- "✅ Confirm" → `backdate_confirm`
- "❌ Cancel" → `backdate_cancel`
...
SUCCESS_BACKDATE_COMPLETED = "✅ <b>Habit backdated:</b> {habit_name}\n📅 <b>Date:</b> {date}"
...
2. Send `/habit_done` → select habit → "Select Date"
3. Verify calendar shows ✓ on completed dates
