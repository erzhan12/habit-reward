...
#### Tables Required
1. **Users**
   - Fields: `telegram_id` (unique), `name`, `weight` (float, multiplier), `active` (bool)
...
4. **Reward Progress**
   - Fields: `user_id` (link to Users), `reward_id` (link to Rewards), `pieces_earned` (int), `status` (enum: 🕒 Pending/⏳ Achieved/✅ Completed), `progress_percent` (formula), `status_emoji` (formula), `actionable_now` (bool)
...
#### Files to Create
- `src/models/user.py` - Pydantic model for User with telegram_id, name, weight, active
...
- `src/services/reward_service.py`
  - `calculate_total_weight(habit_weight: float, user_weight: float, streak_count: int) -> float`
