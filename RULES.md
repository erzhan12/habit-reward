...
**Background Thread Failure Graceful Degradation**: When `_process_login_background` fails (Telegram API down, DB error, etc.), specific exception handlers catch `TelegramError`, `DatabaseError`, and generic `Exception` with differentiated logging. A `wl_failed:{token}` cache key is set so `check_status()` returns `'error'` instead of leaving the user polling `'pending'` indefinitely. The cache entry (`wl_pending:{token}`) was set *before* the thread launched as a fallback. Uses `call_async()` (from `src/web/utils/sync.py`) instead of `asyncio.run()` for consistency with the rest of the codebase. See `TestBackgroundProcessingFailures` and `TestBackgroundProcessingFailure` in `tests/web/test_views.py`.
...
**IP Address Validation**: Client IPs extracted from `X-Forwarded-For` MUST be validated with `ipaddress.ip_address()` before use. Malformed values (e.g. XSS payloads, garbage strings) must fall back to `REMOTE_ADDR`. Use the `_parse_ip_address(request)` helper in `src/web/views/auth.py` — never inline `HTTP_X_FORWARDED_FOR` splitting without validation. The IP appears in the Telegram login notification sent to the user. **X-Forwarded-For is only trusted when `settings.TRUST_X_FORWARDED_FOR=True`** (env: `TRUST_X_FORWARDED_FOR`, default `False`). Enable this only behind a trusted reverse proxy (nginx/Caddy). Tests that need X-Forwarded-For parsing must `@patch("src.web.views.auth.settings")` and set `mock_settings.TRUST_X_FORWARDED_FOR = True`.
